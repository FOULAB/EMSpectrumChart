...
// Set makePDF to true, and change first line of setup, below,
// to write a PDF file
...
//
// Current home: https://github.com/FOULAB/EMSpectrumChart
// Formerly at http://openprocessing.org/sketch/58905
// Something that could be added, from Pierre
// optical bands
...
void setup() {
  // In processing 3, this cannot be controlled by a variable at the top, 
  // have to manually edit
  size(600, 250); assert(!makePDF);
  //size(2500, 250, PDF,"spectrum.pdf"); assert(makePDF);
  // for no display
  ...
  font5 = createFont("DejaVuSans",5);
  font10 = createFont("DejaVuSans",10);
  font15 = createFont("DejaVuSans",15);
  font25 = createFont("DejaVuSans",25);
  fText=font10;
  fExponent=font5;
  fLabel=font15;
  fLabelsmall=createFont("DejaVuSansCondensed",12);//-12.vlw");
  fTitle=font25;
  textFont(fText);
...
public void draw() {
  ...
  text("Chart by Rupert Brooks, VA2WHY. Version 1.1 17-NOV-2019. License CC BY 3.0.",drawingWidth*3.0/4.0-offset,(lBottom+lTop)/2.0-7);
  text("See https://github.com/FOULAB/EMSpectrumChart for more info.",drawingWidth*3.0/4.0-offset,(lBottom+lTop)/2.0+7);
  // display version
  if(makePDF) {
     endRecord();
     // no display version
